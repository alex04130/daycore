...
(function () {
    ...
    return <div className="cj-toasts">{list.map((t) => (
      <div key={t.id} className="cj-toast glass">
        ...
        <div className="bd">
          ...
          <div className="acts">
            {t.opId && <button onClick={() => { if (C.undo(t.opId)) { cjDrop(t.id); cjToast({ label: '已撤销', sub: '原来那笔也留着——撤销也记成一笔', kind: 'info' }); } }}>撤销</button>}
  ...
  function Stack({ onFollow, onGlance }) {
    ...
    const ttlText = (p.ttlRule === 'silent_accept' ? '没理它就当接受' : '没理它就作罢') + ' · ' + (left >= 1 ? Math.round(left) + 'h' : Math.max(5, Math.round(left * 60)) + 'm');
  ...
  function PushChip() {
    ...
    return <React.Fragment>
      ...
      {open && <div className="cj-push-card glass">
        ...
        <div className="row">
          <span className="cj-l0" style={{ marginRight: 'auto' }}>一天最多找你 {core.pushBudget().max} 次</span>

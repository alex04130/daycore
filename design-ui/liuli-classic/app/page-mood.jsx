...
(function () {
  ...
  function MoodPage() {
    ...
    function submit() {
      if (!picked || thinking) return;
      const key = picked;
      const emoji = (MOODS.find(([, k]) => k === key) || ['🙂'])[0];
      setThinking(true);
      ...
      setTimeout(() => {
        const checkin = S.addMood({ mood: emoji + ' ' + t(key), note: noteVal, aiResponse: text, exerciseOffered: ex });
        setThinking(false); setNote(''); setPicked(null);
      ...
      <div className="dc-page-enter" data-screen-label="心情">
        ...
        {picked && !thinking ? (
          <UI.GlassCard className="dc-mt-20">
            <UI.Textarea rows={2} maxLength={200} placeholder={lang === 'zh-CN' ? '想说点什么吗？（可选）' : 'Anything to add? (optional)'} value={note} onChange={(e) => setNote(e.target.value)} />
            <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 10 }}>
              <UI.Button variant="primary" onClick={submit}>{lang === 'zh-CN' ? '记录这一刻' : 'Log this moment'}</UI.Button>
            </div>
          </UI.GlassCard>
        ) : null}
              ...
              {st.moods.slice(0, 6).map((m) => (
                <UI.GlassCard key={m.id} padding="none" className="dc-mood-history-row">
                  ...
                  <div className="dc-mood-hist-main">
                    ...
                    {m.note ? <div className="dc-mood-hist-resp" style={{ color: 'var(--color-text-primary)' }}>{m.note}</div> : null}

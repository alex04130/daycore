        ...
        <div className="dc-mt-20 dc-stack-8">
          ...
          {!st.rules.length ? (
            ...
            <UI.GlassCard key={r.id} padding="none" className={'dc-rule-row' + (r.active ? '' : ' is-paused')}>
              <div className="dc-rule-main" onClick={() => { if (!r.managed) setEditor(r); }} style={r.managed ? { cursor: 'default' } : null}>
                <div className="dc-rule-title">
                  {r.title}
                  <UI.Badge tone="neutral">{t(srcLabel[r.source] || 'rl_source_user')}</UI.Badge>
                  {r.managed ? <UI.Badge tone="neutral">由共享账本管理</UI.Badge> : null}
                  {!r.active ? <UI.Badge tone="neutral">{t('rl_paused')}</UI.Badge> : null}
                </div>
                <div className="dc-rule-desc">{r.managed ? ruleDesc(r) + ' · 在日程里直接改' : ruleDesc(r)}</div>
              </div>
              {r.managed
                ? <span className="dc-rule-lock" title="这条是从已有日程归纳出来的，不是可展开的规则">—</span>

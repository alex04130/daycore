...
(function () {
  ...
  function TodayPage({ nav, tweaks }) {
    ...
    const [sideMonth, setSideMonth] = useState(T.slice(0, 7));
    ...
    const calcBadges = (m) => { const map = {}; for (let i = -31; i < 62; i++) { const d = S.addDays(m + '-01', i); const p = S.getPlan(d); if (p && p.blocks.length) map[d] = {}; } return map; };
    const jumpBadges = useMemo(() => calcBadges(jumpMonth), [jumpMonth, st]);
    const sideBadges = useMemo(() => calcBadges(sideMonth), [sideMonth, st]);
    const upcoming = useMemo(() => st.assignments
      .filter((a) => a.status === 'pending' && a.dueAt)
      .map((a) => ({ ...a, dd: S.dayDiff(a.dueAt.slice(0, 10), T) }))
      .filter((a) => a.dd >= 0 && a.dd <= 14)
      .sort((a, b) => a.dd - b.dd).slice(0, 4), [st]);
    const dueLabel = (dd) => (dd === 0 ? t('ts_due_today') : dd === 1 ? t('ts_due_tomorrow') : t('ts_due_in', { n: dd }));
    ...
    return (
      <div className="dc-page-enter dc-today-grid" data-screen-label="今日">
        <div className="dc-today-main">
        <div className="dc-today-weekstrip"><WeekStrip date={date} onDate={setDate} onJump={() => { setJumpMonth(date.slice(0, 7)); setJumpOpen(true); }} /></div>
        ...
        <aside className="dc-today-side">
          <div className="dc-side-card"><Calendar month={sideMonth} onMonth={setSideMonth} value={date} onPick={setDate} badges={sideBadges} /></div>
          {blocks.length ? (
            <div className="dc-side-card">
              <div className="dc-side-head"><IC.CheckCircle size={15} /> {t('ts_progress')}<span className="num">{doneCount}/{blocks.length}</span></div>
              <div className="dc-ts-bar"><i style={{ width: Math.round((doneCount / blocks.length) * 100) + '%' }}></i></div>
            </div>
          ) : null}
          <div className="dc-side-card">
            <div className="dc-side-head"><IC.Clock size={15} /> {t('ts_upcoming')}</div>
            {upcoming.length ? (
              <div className="dc-side-list">
                {upcoming.map((a) => (
                  <button key={a.id} type="button" className="dc-side-due" onClick={() => nav.go('materials')}>
                    <span className="tt">{a.title}</span>
                    <span className={'dd' + (a.dd <= 1 ? ' is-hot' : a.dd <= 3 ? ' is-warm' : '')}>{dueLabel(a.dd)}</span>
                  </button>
                ))}
              </div>
            ) : <p className="dc-side-empty">{t('ts_none')}</p>}
          </div>
        </aside>
      </div>
    );

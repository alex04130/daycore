// Daycore v2.2 — app shell: tab nav, appbar, theme boot, tweaks
(function () {
  'use strict';
  ...
  function App() {
    ...
    const [tweaks, setTweak] = useTweaks({ apVariant: 'brief', showAiNote: true, showConfidence: true });
    ...
    return (
      <div className="dc-app">
        <header className={'dc-appbar' + (tab === 'today' ? ' is-wide' : '')}>
        ...
        <main className={'dc-main' + (tab === 'today' ? ' is-wide' : '')}>
          {tab === 'today' ? <P.TodayPage nav={nav} tweaks={tweaks} /> : null}
          {tab === 'materials' ? <P.MaterialsPage nav={nav} tweaks={tweaks} /> : null}
        ...
        <aside className="dc-rail">
          ...
          <nav className="dc-rail-nav">
            {TABS.map((it) => (
              <button key={it.key} type="button" className={'dc-rail-item' + (tab === it.key ? ' is-active' : '')} onClick={() => nav.go(it.key)}>
                {it.icon}<span className="dc-rail-item-label">{it.label}</span>
              </button>
            ))}
          </nav>
          <button type="button" className="dc-rail-user" onClick={() => setAccountOpen(true)}>
            <UI.Avatar name={st.user ? st.user.name : '?'} size={34} />
            <span className="dc-rail-user-meta">
              <span className="name">{st.user ? st.user.name : t('au_anon')}</span>
              <span className="sub">{st.user ? t('au_synced') : t('au_signin')}</span>
            </span>
        ...
        <TweaksPanel title="Tweaks">
          ...
          <TweakSection label="随手记" />
          <TweakToggle label="显示 AI 置信度" value={tweaks.showConfidence} onChange={(v) => setTweak('showConfidence', v)} />
        </TweaksPanel>
      </div>
    );
  }

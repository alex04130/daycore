// Ported 1:1 from the design prototype (app/ui.jsx): hooks, toast, sheets,
// calendar, overlays, theme application.
...
export function applyThemeVars(themeObj) {
...
export function applyCurrentTheme(state) {
...
// ---------- Confirm ----------
export function Confirm({ open, onClose, title, body, confirmLabel, danger, onConfirm }) {
  return (
...
export function Field({ label, sub, children, inline }) {
  return (
...
export function Calendar({ month, onMonth, value, range, onPick, min, max, badges }) {
  ...
  const wds = I18N.weekdayNames('narrow');
  const T = S.todayIso();
  ...
  return (
    <div className="dc-cal">
      <div className="dc-cal-head">
        ...
        <span className="dc-cal-title">{I18N.fmtDate(month + '-01', 'md').replace(/日|1(?=,|\s|$)/, '').replace(/\s?1$/, '') || month}</span>
...
export function WeekStrip({ date, onDate, onJump }) {
  const T = S.todayIso();
  ...
  const wds = I18N.weekdayNames('narrow');
  useEffect(() => { S.ensurePlans(start, S.addDays(start, 6)); }, [start]);
  return (
...
export function GenOverlay({ open, phases, done, onDone }) {
...
export const typeLabel = (type) => t('type_' + type);

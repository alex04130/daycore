...
- **角色轨能不能碰 `db/*` 与 `users/*`**。审查给了两个相反答案，而危险的那个更简单、会赢：可浏览表清单里有 `chat_messages` / `mood_checkins` / `memory_facts` —— 与陪伴边界同级的信任数据。倾向是「口令轨独占」。
- **谁能授予**（授予端点本身归哪个权限）。
- **破窗轨能不能撤销管理员**。审查指出：若「破窗只授不撤」+「角色轨不能授予」同时成立，**没有任何人能移除一个管理员**。倾向是破窗轨可授可撤 —— `ADMIN_TOKEN` 在生产必然存在（未设直接拒绝启动），所以合法所有者手上永远有同一把钥匙。

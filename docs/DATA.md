...
`DeliverAfter` / `DeliveredAt` 在，共识 15 描述的池子也在，但**把排队的卡变成堆叠里的卡**那个调度器不存在 —— 现在建它就是本仓一再犯的那个错：一个没有生产者的机制。

今天**没有任何东西排队**：每个生产者都在创建时就盖 `DeliveredAt`，因为每一个都是对用户刚做的事的回应（一轮对话里的决策卡、他问出来的冲突、第 20 小时的关怀）。一张造出来就在屏幕上的卡不需要调度器。

**什么时候建**：第一个真正排队的生产者出现时 —— EXPERIENCE_CORE §12.2 的 daemon 产出链（习惯扫描 → Rule 提案、愿望池填缝）就是那个，它产出的是没人要过的卡、在没人选过的时刻。那时「这个该什么时候给他看」才成为真问题，答案要的是共识 15 的重排/合并/背压，而不是一次时间戳比较。

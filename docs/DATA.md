...
⚠️ **90 天有代价，写下来免得以后当 bug 查**：`AdminStats` 对这张表做 COUNT 和 SUM，所以总览上的「AI 调用」「Token 消耗」**是窗口内的数字，不是全时段的，而且会往下掉**。这是诚实的（一个只涨不落的全时段计数器没人能拿它做决定），但界面上必须标出来 —— 已标。真要更长的账期，做法是清理前先写一份月度汇总，**不是把保留期拉长**：为了算一个和而永久保留每一行是最贵的存法。

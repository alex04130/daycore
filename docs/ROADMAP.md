...
⬜ 余下的 F7：**按 family 的主题存储**（`custom_themes.family_id` —— 今天主题仍挂在那份写死的 13 条上）、补算作业、控制台那一屏（批准 rules、批准第三档 pattern、挪 build、钉 family）。**DB 层的 kind 也等那一屏**：`Registry` 已经收 `OriginDB`，但生产代码没有人用它调 `Merge`，这是有意的 —— 一个存 kind 的地方而没有存进去的路，就是本仓犯过七次的那种半成品。

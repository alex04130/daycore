...
- ✅ **`POST /api/admin/restart`**（2026-08-10）—— **真的重启，不是退出等别人拉**。
  ...
  **选了「起新进程再退出自己」**，因为要求是「Windows + Linux 一致」：`exec` 自身在 Unix 上更干净，但 **Windows 没有它**，那边必须走另一条 —— 于是两份实现、两种失败模式，而其中一份只有碰巧在那个平台上跑的人才会踩到。一条到处一样的路，胜过两条各自更简单的路。

  **端口交接靠「子进程重试 bind」，不靠 `SO_REUSEADDR`** —— 后者在两个平台语义不同，而且正好差在要命的方向（Windows 上它允许第二个 socket **抢走**一个活着的监听）。子进程重试 30 秒，父进程优雅停机时关掉监听，下一次尝试就成了。**重试只在重启时武装**（父进程传 `DAYCORE_RESTART_FROM`），普通启动仍然端口被占就立刻失败 —— 沉默十秒再报「address already in use」比立刻报差得多。
  ...
  **真机 e2e 已跑**（`cmd/daycore/restart_e2e_test.go`）：编译真二进制、真的起、真的点、断言 **instance id 变了**（只断言「还活着」的话，一个答 200 然后什么都不做的实现同样能过）。三条断言都靠改坏实现红过。测试无 build tag、无 `runtime.GOOS` 分支 —— 哪天它需要一个，就是「单一路径」这个性质丢了的信号。

...
⬜ 余下的 F7：family / build 两层身份、`POST /api/version` 握手、按 family 的 token 空间取并集、补算作业、控制台里批准第三档 pattern 的那一屏（DB 层也在那时接上 —— 现在 `Registry` 已经收 `OriginDB`,但生产代码没有人用它调 `Merge`,这是有意的:一个存 kind 的地方而没有存进去的路,就是本仓犯过七次的那种半成品）。

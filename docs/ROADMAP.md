...
- ⬜ `POST /api/admin/restart` —— 优雅重启自己。⚠️ **它只在有 supervisor 的部署里有意义**（systemd / docker restart policy / k8s）：裸跑的进程退出之后没有人把它拉起来。所以这个按钮必须先问「你有没有 supervisor」，或者干脆检测不到就禁用并说明 —— 一个把服务器关掉再也起不来的按钮是这个控制台能犯的最贵的错。

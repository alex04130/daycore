...
|分区|状态|
|---|---|
|总览|⬜ 「最近的 AI 调用」那张卡没有后端（`AICallLogRepository` 没有 `List`）|
|服务配置|✅|
|能力源|✅（随 F2-A）|
|模型 / 第三方登录|✅ 只读，理由见上一条|
|提示词|⬜|
...
|数据库|⬜ `db/backup` 是 501，`export`/`import` 是桩；按表拆 `db.user_content` 未做|
...
- ✅ **权限本体**（2026-08-10，见 [AUTH.md](AUTH.md)「权限本体：落地形状」）—— 20 条权限 + 角色/成员四后端存储 + `adminGate` 路由层检查 + 授予端点 + 用户与权限屏。作者的九条裁决全部有对应代码与**红过的**断言。
  ...
  ⬜ 还欠：`db.user_content` 的按表拆分（常量已定义、**有意不注册**，因为反向闸门要求每条注册过的权限至少被一条路由引用 —— 一个什么都不授予的开关比没有更糟）。

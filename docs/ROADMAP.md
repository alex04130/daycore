...
⬜ 汀 还没做的（写在它自己的 README 里）：导语面 / 心情帧 / 全天抽屉 / 账本（需要还没调的端点，**不做假的** —— 接在 mock 形状上的界面是注定要重写的界面）、多语言（四条要求一条都没实现，写死中文，已知欠账）、主题切换入口。

⬜ 之后：琉璃 ∥ 纸屿 ∥ 运营台 → 阶段 κ 切子仓。**DB 层的 kind 仍然没有写入的路**：`Registry` 已经收 `OriginDB`，但生产代码没有人用它调 `Merge`，这是有意的 —— 一个存 kind 的地方而没有存进去的路，就是本仓犯过七次的那种半成品。

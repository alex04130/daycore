...
- ✅ **剩下四屏**（2026-08-10，见 [DATA.md](DATA.md)）—— 总览 / 提示词 / AI 日志 / 数据库。三件事值得单独记：
  ...
  1. **AI 账本第一次可读也可清。** `List`（五维过滤 + 向后 keyset 翻页）与 `Prune`（90 天，leader 上跑）。加 Prune 不是顺手：这是库里长得最快的表，而**此前没有任何东西从它删过** —— `proposals` 当初那个洞的同一个形状。⚠️ 代价写在 DATA.md：总览的「AI 调用 / Token 消耗」因此是窗口内的数字，**会往下掉**，界面上已标。

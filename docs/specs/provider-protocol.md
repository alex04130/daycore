...
> 状态：**查询型（天气 / 搜索）的 `format: http` 已实现并有可运行的参考实现**（2026-08-09，F2-A）。通道型与 `format: exec` 仍是 ⬜ 未实现。
>
> **`tools/adapter-example/` 是照抄的起点** —— 一个说这套协议的独立程序，`go run ./tools/adapter-example` 就能跑起来。`internal/adapters` 的 e2e 把它当真子进程启动、走真 socket，所以「照这份文档写的适配层能用」是被验证过的，不是被声称的。
>
> ⚠️ 那条 e2e 落地当天就抓到一个**规范本身的错**：`base_url` 曾经禁止 loopback，理由是「127.0.0.1 上的适配层与本服务器的管理面无法区分」。那条理由经不起推敲 —— 能改 `providers.yaml` 的人本来就有 shell，可以直接从 `.env` 里读 `ADMIN_TOKEN`。禁令什么都没买到，却把最重要的那个场景（同机部署、以及跑参考实现本身）挡在门外。**现在只禁 link-local**（云 metadata 端点，那里没有任何合法的东西）。
>

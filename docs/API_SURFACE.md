...
>
> **2026-07-29 起路由分散注册**：本表说的「Handler 文件」同时也是**注册它的地方** —— 每个文件在自己的 `init()` 里 `registerRoutes(...)`，不再有一份集中清单（见 ARCHITECTURE.md「路由注册模式」）。所以「路由在哪个文件」这个问题现在有两个答案且必然一致。
>
> 与 openapi.yaml 的一致性已由 `internal/server/routes_test.go` **双向**强制（服务了没写进契约 / 写进契约没人服务，都是测试失败）。本表仍是手写的，只有它会脱节。当前实际：**100 条路由 / 21 个组**。
...
| GET /api/version（API 契约版本，2026-07-14 起；**2026-07-26 加 `locales`：`available` 装了哪些语言 + `defaultPrimary`/`defaultSecondary` 新用户默认值。用户自己的一主一副在 `/api/session/preferences`**） | handlers_misc.go |

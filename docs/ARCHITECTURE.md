...
- **删掉了 `-tags lite`。** 它宣称「不内嵌文件、二进制更小」，两样都不成立：`//go:embed` 在 `prompts.go`，那个文件**没有 build tag**，模板照样编进去；实测差 20 KiB / 23.5 MB（0.08%），全是那个访问器本身。而它真正做到的是让 `WalkPromptFS` 变成 no-op，于是 `daycore install` 在它下面解压零个模板 —— **唯一可观察的效果是把安装命令弄坏。** 它还与已经写进 `docs/DATA.md` 的「内嵌是地板」直接矛盾：地板存在的意义就是没有数据库、没有磁盘文件时二进制仍能起来并说清哪里不对，为 20 KiB 拆掉地板不是这个项目做得起的交易。

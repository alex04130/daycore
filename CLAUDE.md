...
- **存储层改动的验收标准是 `internal/storage/storagetest` 的行为套件**（29 个用例，SQLite 与真机 Mongo 跑同一份）。它测的是四个后端必须一致的**行为**，`dialect_parity_test.go` 是三方言 DDL 的**静态**比对，两者互不替代。本机跑真机那半：`make test-mongo`。
...
- CI：`.github/workflows/ci.yml` 四个 job —— backend（gofmt 门禁 + vet + test + **真机 MongoDB service，跑存储行为一致性套件**，并断言它没有静默 skip + 静态二进制）、frontend（i18n 校验 + vite build）、extension（MV3 manifest 与双 locale 校验 + 打 zip）、docker（构建镜像）。**改动后本地先跑 `gofmt -l .` 确认为空**，否则 CI 直接红。

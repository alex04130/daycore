...
- **路由注册模式**：每个 handler 文件在自己的 `init()` 里 `registerRoutes("<组名>", func(s *Server, mux Mux){…})`，注册表在 `internal/server/routes.go`。**不要去 `server.go` 加路由** —— 那里只剩静态 `/` 一条。`RouteTable(*Server)` 用零值 Server 重放注册（读路由表不需要数据库）。当前 **102 条路由 / 22 个组**（`docs/API_SURFACE.md` 是生成物，`make api-surface` 重生成）。
...
- **版本三层，不要混**（唯一真源 `internal/version/version.go`，同步 `web/frontend/package.json`）：
  ...
  2. **API 契约版本** `APIVersion=1` / `APIMinor=4`（`GET /api/version`；各前端握手用这个）：breaking 升 major，additive 升 minor；契约面变了必须升版，由 `api/spec/contract-lock.json` + `go test` 强制。
